...
def make_optimizer(model, alpha=0.0002, beta1=0., beta2=0.999):
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default='/home/DataB/datasets_max/ImageNet')
    ...
    parser.add_argument('--stat_file', type=str, default='./datasets/inception_model/fid_stats_imagenet_train.npz',
                        help='path to the inception model')

    parser.add_argument('--snapshot', type=str, default='snapshot_iter_40000',
    ...
    opt_gen = make_optimizer(
        models['main']['gen'], alpha=config.adam_G['alpha'], beta1=config.adam_G['beta1'], beta2=config.adam_G['beta2'])
    opt_dis = make_optimizer(
        models['main']['dis'], alpha=config.adam_D['alpha'], beta1=config.adam_D['beta1'], beta2=config.adam_D['beta2'])
    ...
    trainer.extend(calc_inception(gen, n_ims=5000, splits=1, dst=args.results_dir, path=args.inception_model_path),
                   ...
                   priority=extension.PRIORITY_WRITER)
    trainer.extend(
        calc_FID(gen, n_ims=5000, dst=args.results_dir, path=args.inception_model_path, stat_file=args.stat_file),
        trigger=(config.evaluation_interval, 'iteration'),
        priority=extension.PRIORITY_WRITER)
    ...
    ext_opt_gen = extensions.LinearShift('alpha', (config.adam_G['alpha'], 0.),
                                         (config.iteration_decay_start, config.iteration), opt_gen)
    ext_opt_dis = extensions.LinearShift('alpha', (config.adam_D['alpha'], 0.),
