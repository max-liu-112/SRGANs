...
### SR method
Actually, SR method only adds few modification based on SN method. In this example, we do image generation on ImageNet. After the gradient update, we conduct dynamic compensation, which is implemented based on the 'SR' method of the Updater Class in https://github.com/max-liu-112/SRGANs/blob/master/updater.py. In the next forward process, we use weight matrix w to do the convolutional operation, then the w has already been compensated. 
