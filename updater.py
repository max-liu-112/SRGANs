...
class Updater(chainer.training.ParallelUpdater):
    def __init__(self, *args, **kwargs):
        ...
        self.apply_SR = kwargs.pop('apply_SR')
        self.n_accumulation =  kwargs.pop('num_accumulation')
        self.recoreded_sigma = None
        self.n_SR = 5
        ...
        super(Updater, self).__init__(devices=self.devices, models=self.models, iterator=kwargs.pop('iterator'),
    ...
    def get_batch(self, xp):
        ...
        #print(x_real.data.device)
        return x_real, y_real
    # implemenation of Spectral Regularization
    def SR(self, dis):
        if self.recoreded_sigma == None:
            self.recoreded_sigma ={}
            ...
            for name, link in links:
                if isinstance(link, SNConvolution2D):
                    ...
                    self.recoreded_sigma[name] = s/max(s)
        else:
            links = [[name, link] for name, link in sorted(dis.namedlinks())]
            for name, link in links:
                if isinstance(link, SNConvolution2D):
                    ...
                    sigma1 = max(s)
                    s = s / sigma1
                    self.recoreded_sigma[name] = np.maximum(s, self.recoreded_sigma[name])
                    compensation_s = sigma1 * self.recoreded_sigma[name]
                    S = np.diag(compensation_s)
                ...
                for name in names:
                    with function.force_backprop_mode():
                        ...
                        with cuda.get_device_from_id(dev_id):
                            ...
                            x_fake, y_fake = self._generete_samples(gen, batchsize)
                            ...
                            loss_dis.backward()
            # copy the gradients to main model
            for name in names:
                if name != 'main':
            ...
            if i==0:
                for accumulation_index in range(self.n_accumulation):
                    for name in names:
                        with function.force_backprop_mode():
                            dev_id = self.devices[name]
                            dev_id = dev_id if dev_id >= 0 else None
                            with cuda.get_device_from_id(dev_id):
                                gen = self.models[name]['gen']
                                dis = self.models[name]['dis']
                                x_fake, y_fake = self._generete_samples(gen)
                                dis_fake = dis(x_fake, y=y_fake)
